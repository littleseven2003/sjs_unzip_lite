...
pub async fn run_task(ctx: &mut TaskContext, app: &AppHandle) -> Result<(), AppError> {
    ...
    log_writer.write_header(&ctx.root_dir.to_string_lossy(), &ctx.final_folder_name)?;
    ...
    match run_task_inner(ctx, app, &log_writer).await {
        ...
        Err(e) => {
            ...
            emit_log_and_file(
                app,
                &log_writer,
                LogEvent::error(format!("处理失败：{}", e), None),
            );
            log_writer.write_footer(false)?;
            Err(e)
        }
...
async fn run_task_inner(
    ...
    emit_log_and_file(
        app,
        log_writer,
        LogEvent::info("开始扫描文件夹", Some(ctx.root_dir.display().to_string())),
    );
    ...
    emit_log_and_file(
        ...
        LogEvent::info(
            format!(
                "找到分卷组：{}，共 {} 个文件",
                volume_group.base_name,
                volume_group.files.len()
            ),
            None,
        ),
    );
    ...
    emit_log_and_file(
        app,
        log_writer,
        LogEvent::success("分卷文件已移动到根目录", None),
    );

    // 清理空文件夹
    emit_progress(
        app,
        TaskStatus::CleaningFolders,
        "正在清理空文件夹",
        25,
        None,
    );
    ...
    if !removed.is_empty() {
        emit_log_and_file(
            app,
            log_writer,
            LogEvent::info(format!("已清理 {} 个空文件夹", removed.len()), None),
        );
    }

    // 清理额外文件（在解压前清理，避免解压后同名文件被误删）
    let volume_file_names: Vec<String> = volume_group
        .files
        .iter()
    ...
    if !extra_files.is_empty() {
        ...
        emit_log_and_file(
            app,
            log_writer,
            LogEvent::info(format!("已清理 {} 个额外文件", cleaned_count), None),
        );
    }
    ...
    emit_progress(
        app,
        TaskStatus::Extracting7z,
        "正在解压 7z 分卷压缩包",
        35,
        None,
    );
    let first_volume = ctx
        .root_dir
        .join(format!("{}.7z.001", volume_group.base_name));
    ...
    let password_index =
        extractor::extract_with_passwords(app, &first_volume, &ctx.root_dir, &ctx.password_list)
            .await?;
    ...
    for iteration in 1..=ctx.max_iterations {
        ...
        emit_log_and_file(
            app,
            log_writer,
            LogEvent::info(format!("找到 txt 文件：{}", txt_file.display()), None),
        );

        // 改名 txt → rar
        emit_progress(
            app,
            TaskStatus::RenamingTxtToRar,
            "正在将 txt 改名为 rar",
            55,
            None,
        );
        ...
        emit_log_and_file(
            app,
            log_writer,
            LogEvent::success(format!("已改名为：{}", rar_file.display()), None),
        );

        // 校验压缩包有效性
        emit_progress(
            app,
            TaskStatus::CleaningExceptRar,
            "正在校验压缩包",
            60,
            None,
        );
        ...
        emit_progress(
            app,
            TaskStatus::ExtractingRar,
            "正在解压 rar 文件",
            70,
            None,
        );
        let password_index =
            extractor::extract_with_passwords(app, &rar_file, &ctx.root_dir, &ctx.password_list)
                .await?;
    ...
    if !ctx.final_folder_name.is_empty() {
        ...
        if current_name != ctx.final_folder_name {
            ...
            emit_log_and_file(
                app,
                log_writer,
                LogEvent::success(format!("已重命名为：{}", new_path.display()), None),
            );
        }
    }
